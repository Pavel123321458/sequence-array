...
protected:
    ...
    virtual ArraySequence<T>* CreateInstance(T* items, int count) const = 0;
...
public:
    ...
    virtual T GetFirst() const override {
        if (array->GetSize() == 0)
            throw std::out_of_range("Index out of range: sequence is empty");
        return array->Get(0);
    }
    ...
    virtual Sequence<T>* GetSubsequence(int startIndex, int endIndex) const override {
        ...
        Sequence<T>* result = CreateInstance(items, newSize);
        delete[] items;
        return result;
    }
