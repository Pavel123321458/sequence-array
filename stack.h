...
public:
    Stack() : data(new MutableArraySequence<T>()) {}
    ...
    if (len == 1) {
        delete data;
        data = new MutableArraySequence<T>();  // пустая последовательность
    } else {
    ...
    Stack<T>* Map(T (*f)(const T&)) {
        ...
        Stack<T>* result = new Stack<T>(new MutableArraySequence<T>(items, len));
        delete[] items;
        return result;
    }
    ...
    Stack<T>* Where(bool (*pred)(const T&)) {
        ...
        Stack<T>* result = new Stack<T>(new MutableArraySequence<T>(items, newLen));
        delete[] items;
        return result;
    }
